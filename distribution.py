...
if args.shards != None:
    ...
    if args.distribution == "uniform":
        ...
        with open("containers/{}/splitfile.npy".format(args.container), "wb") as f: np.save(f, np.array(partition, dtype=object), allow_pickle=True)
        requests = np.array([[] for _ in range(args.shards)])
        #np.save("containers/{}/requestfile:{}.npy".format(args.container, args.label),requests,)
        with open("containers/{}/splitfile.npy".format(args.container), "wb") as f: np.save(f, np.array(partition, dtype=object), allow_pickle=True)
    ...
    # Else run PLS-GAP algorithm to find a low cost split.
        ...
        if args.shards != None:
            ...
            #np.save("containers/{}/requestfile:{}.npy".format(args.container, args.label),requests,)
            with open("containers/{}/requestfile:{}.npy".format(args.container, args.label), "wb") as f: np.save(f, np.array(requests, dtype=object), allow_pickle=True)
...
if args.requests != None:
    if args.distribution == "reset":
        ...
        # Load splitfile.
        #partition = np.load("containers/{}/splitfile.npy".format(args.container), allow_pickle=True)
        with open("containers/{}/splitfile.npy".format(args.container), "rb") as f: partition = np.load(f, allow_pickle=True)
        ...
        # Update requestfile.
        #np.save("containers/{}/requestfile:{}.npy".format(args.container, args.label),np.array(requests),)
        with open("containers/{}/requestfile:{}.npy".format(args.container, args.label), "wb") as f: np.save(f, np.array(requests, dtype=object), allow_pickle=True)
